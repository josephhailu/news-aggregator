...
This uses `docker-compose.yml` plus `docker-compose.dev.yml`. PostgreSQL and Ollama still run as normal containers, while `api` and `web` run in watch mode with the workspace mounted in so frontend and backend edits apply without rebuilding images on every change.
...
sequenceDiagram
  ...
  API->>DB: Get cached article text if fresh
  API->>API: Fetch and clean article HTML if needed
...
Text extraction is cached in `article_texts`.
...
- HTML extraction is regex-based and intentionally simple; it is acceptable for the first Fed use case but not a robust general article parser. User-submitted URLs are still deferred; before enabling them, add URL allowlisting, private-network blocking, redirect limits, and response size limits.
